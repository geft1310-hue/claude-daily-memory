...
## Экономия токенов — главный приоритет

Каждый вызов нейросети пересылает весь контекст сессии — это деньги пользователя. Впустую жечь токены нельзя.

- Несколько агентов/сессий параллельно — только при явной необходимости.
- Не дробить работу на мелкие шаги: независимые проверки объединять в один вызов.
- Читать файлы частями (`Read` с `limit`/`offset`), искать точечно (`grep`/`find`), не тянуть целиком.
- Не тащить в контекст простыни отчётов и дампов; от агентов требовать сжатую выжимку.
- Дорогие команды (сборки, полные тесты) повторно не гонять, если результат уже получен.
- Анализ большого проекта: сначала карта, углубляться только в нужные места.
- Длинную сессию сжимать (`/compact`) или начинать новую на новую задачу.
- Отвечать коротко: результат и то, что важно.
